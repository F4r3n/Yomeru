...
import { putCard, getCard, getAllCards, getDueCards, deleteCard, addLookupHistory } from "./idb.ts";
...
browser.runtime.onMessage.addListener((msg: { type: string; payload?: unknown }) => {
  switch (msg.type) {
    case "ADD_WORD":    return handleAddWord(msg.payload as { word: string; reading: string; meaning_en: string });
    case "REVIEW_CARD": return handleReviewCard(msg.payload as { word: string; rating: number });
    case "GET_DUE":    return handleGetDue();
    case "GET_ALL_CARDS": return handleGetAllCards();
    case "DELETE_CARD": return handleDeleteCard(msg.payload as { word: string });
    case "LOG_LOOKUP":  return handleLogLookup(msg.payload as { word: string; reading: string });
    default: return Promise.resolve({ error: "Unknown message type" });
  }
});

async function handleAddWord({ word, reading, meaning_en }: { word: string; reading: string; meaning_en: string }) {
  await ensureSrs();
  if (await getCard(word)) return { success: true, existing: true };
  const card = srs!.new_card(word, reading, meaning_en ?? "", Date.now()) as SrsCard;
  await putCard(card);
  return { success: true, existing: false };
}

async function handleReviewCard({ word, rating }: { word: string; rating: number }) {
  await ensureSrs();
...
async function handleLogLookup({ word, reading }: { word: string; reading: string }) {
