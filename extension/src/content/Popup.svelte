<script lang="ts">
  ...
  let entriesKey = $derived($popupStore.entries.map((e) => e.sequence).join(","));
  ...
  async function addToSrs(word: string, rdg: string, meaning: string) {
    ...
    buttonStates = { ...buttonStates, [word]: res.existing ? "existing" : "added" };
  }
</script>
      ...
      <div class="jp-entry">
        ...
        <div class="jp-senses">
          {#each entry.senses.slice(0, 3) as sense, si}
            {@const g = sense.glosses.filter((g) => g.lang === "eng").map((g) => g.text).join("; ")}
            {#if g}
              <div class="jp-gloss">
        ...
        <button
          ...
          onclick={() => btnState === "idle" && addToSrs(hw, rdg, gloss)}
        >
