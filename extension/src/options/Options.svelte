<script lang="ts">
  ...
  let statsText = $derived(`${dueCards.length} card${dueCards.length !== 1 ? "s" : ""} due`);
  ...
  async function rate(rating: number) {
    ...
    await browser.runtime.sendMessage({ type: "REVIEW_CARD", payload: { word: currentCard.word, rating } });
    currentIdx++;
    showBack = false;
  ...
  async function computeNextDue() {
    ...
    if (next < Infinity) {
      ...
      nextDueMsg = mins < 60 ? `Next card due in ${mins} min` : `Next card due in ${Math.round(mins / 60)} hr`;
    }
  }
...
<main>
    ...
    {:else}
      <div class="card">
        ...
        <div class="card-actions">
          {#if !showBack}
            <button class="btn-show" onclick={() => (showBack = true)}>Show answer</button>
          {:else}
  ...
  {:else}
    <div class="word-list-header">
      ...
      <input class="word-search" type="search" placeholder="Search…" bind:value={searchQuery} />
    </div>
    <table class="word-table">
      <thead>
        <tr><th>Word</th><th>Reading</th><th>Meaning</th><th>Due</th><th></th></tr>
      </thead>
      <tbody>
...
<style>
  :global(:root) {
    ...
    --green: #a6e3a1; --red: #f38ba8; --yellow: #f9e2af; --blue: #89dceb;
  }
  :global(*) { box-sizing: border-box; margin: 0; padding: 0; }
  :global(body) {
    background: var(--bg); color: var(--text);
    font-family: "Noto Sans JP", "Segoe UI", sans-serif;
    font-size: 14px; min-width: 440px; min-height: 300px; padding: 0 0 16px;
  }

  header { background: var(--surface); padding: 12px 16px 0; border-bottom: 1px solid var(--border); }
  h1 { font-size: 16px; font-weight: 700; color: var(--blue); margin-bottom: 10px; }
  nav { display: flex; gap: 4px; }

  .tab {
    background: none; border: none; border-bottom: 2px solid transparent;
    color: var(--subtext); cursor: pointer; padding: 6px 12px; font-size: 13px;
  ...
  .review-stats { font-size: 13px; color: var(--subtext); margin-bottom: 12px; }

  .card {
    background: var(--surface); border: 1px solid var(--border);
    border-radius: 10px; padding: 24px 20px 16px; text-align: center;
  }
  .card-word    { font-size: 42px; font-weight: 700; color: var(--blue); margin-bottom: 8px; }
  .card-reading { font-size: 18px; color: var(--green); margin-bottom: 6px; }
  .card-meaning { font-size: 16px; color: var(--text); margin-bottom: 4px; }
  .card-actions { margin-top: 20px; }

  .btn-show {
    background: var(--accent); border: none; border-radius: 6px;
    color: var(--bg); cursor: pointer; font-size: 14px; font-weight: 600;
    padding: 8px 32px; transition: opacity 0.15s;
  }
  ...
  .rating-btn {
    ...
    font-size: 13px; font-weight: 600; padding: 7px 16px; transition: opacity 0.15s;
  }
  ...
  .review-done { text-align: center; padding: 32px; color: var(--subtext); }
  .next-due { margin-top: 8px; color: var(--accent); }

  .word-list-header {
    display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;
  }
  .word-count { color: var(--subtext); font-size: 13px; }
  .word-search {
    background: var(--surface); border: 1px solid var(--border);
  ...
  .word-table th {
    ...
    border-bottom: 1px solid var(--border); color: var(--subtext); font-weight: 500;
  }
  .word-table td { padding: 6px 8px; border-bottom: 1px solid var(--border); vertical-align: top; }
  ...
  .btn-delete {
    ...
    border-radius: 4px; color: var(--red); cursor: pointer;
    font-size: 11px; padding: 2px 8px;
  }
  .btn-delete:hover { background: var(--surface); }
</style>
